...
const getBalances = async (): Promise<
  AxiosResponse<IBalanceUpdatePayload[], any>
> => {
...
const getBalanceById = async (
  id: string
): Promise<AxiosResponse<IBalanceUpdatePayload, any>> => {
  return await httpCommon.get<IBalanceUpdatePayload>(`${Route}/${id}`);
};

const updateBalance = async (
  id: string,
  balance: IBalanceUpdatePayload
): Promise<AxiosResponse<IBalanceUpdatePayload, any>> => {
  return await httpCommon.post<IBalanceUpdatePayload>(
    `${Route}/${id}`,
    balance
  );
};
